const blogPosts = [
    {
      id: 1,
      title: "The Future of AI: What to Expect in 2030",
      subtitle: "Exploring the next wave of artificial intelligence advancements.",
      postImage: "https://plus.unsplash.com/premium_photo-1683121710572-7723bd2e235d?q=80&w=1932&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      author: {
        fullName: "Noman Haider",
        profileImage: "https://source.unsplash.com/100x100/?person,portrait",
      },
      comments: [
        {
          id: 1,
          commenter: "Ali Khan",
          comment: "This is an exciting time for AI. Can't wait to see what happens next!",
          postedOn: "2024-12-01",
        },
        {
          id: 2,
          commenter: "Sarah Ahmed",
          comment: "Great article! I found it very insightful.",
          postedOn: "2024-12-02",
        },
      ],
    },
    {
      id: 2,
      title: "Top 10 Travel Destinations for 2024",
      subtitle: "Your ultimate guide to exploring the world in the coming year.",
      postImage: "https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?q=80&w=1770&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      author: {
        fullName: "Sophia Malik",
        profileImage: "https://source.unsplash.com/100x100/?woman,portrait",
      },
      comments: [
        {
          id: 1,
          commenter: "John Doe",
          comment: "Adding these to my bucket list!",
          postedOn: "2024-12-01",
        },
        {
          id: 2,
          commenter: "Emily Clark",
          comment: "Thanks for sharing this! Amazing recommendations.",
          postedOn: "2024-12-02",
        },
      ],
    },
    {
      id: 3,
      title: "Mastering React: Tips for Beginners",
      subtitle: "Learn how to build dynamic user interfaces with ease.",
      postImage: "https://images.unsplash.com/photo-1633356122102-3fe601e05bd2?q=80&w=1770&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      author: {
        fullName: "Ahmed Raza",
        profileImage: "https://source.unsplash.com/100x100/?man,developer",
      },
      comments: [
        {
          id: 1,
          commenter: "Zara Muhammad Iqbal",
          comment: "This was very helpful. Thank you!",
          postedOn: "2024-12-01",
        },
      ],
    },
    {
      id: 4,
      title: "Healthy Habits for a Productive Day",
      subtitle: "Start your day right with these simple routines.",
      postImage: "https://images.unsplash.com/photo-1511909525232-61113c912358?q=80&w=1712&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      author: {
        fullName: "Fatima Sheikh",
        profileImage: "https://source.unsplash.com/100x100/?woman,fitness",
      },
      comments: [
        {
          id: 1,
          commenter: "Ali Khan",
          comment: "I've been following these tips, and they've made a difference!",
          postedOn: "2024-12-02",
        },
      ],
    },
    {
      id: 5,
      title: "The Science Behind Climate Change",
      subtitle: "Understanding the data and what it means for our future.",
      postImage: "https://plus.unsplash.com/premium_photo-1661432575489-b0400f4fea58?q=80&w=1772&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      author: {
        fullName: "Ayesha Karim",
        profileImage: "https://source.unsplash.com/100x100/?person,scientist",
      },
      comments: [
        {
          id: 1,
          commenter: "Daniel Lee",
          comment: "This was very informative. Thank you for raising awareness.",
          postedOn: "2024-12-01",
        },
        {
          id: 2,
          commenter: "Maria Khan",
          comment: "We need to act now. Thanks for this important read.",
          postedOn: "2024-12-02",
        },
      ],
    },
  ];
  
  export default blogPosts;
  